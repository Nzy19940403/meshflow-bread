...
# More aggressive wholesale cuts
tests=[(0.40,0.85,1.5),(0.38,0.90,1.8),(0.35,1.0,2.0)]

for label, (ws_p,ws_i,ws_pk) in tests:
    best=[-1e9,None,None]
    for B1 in[16,18,20]:
     for B24 in[10,12,14]:
      for B25 in[0,100]:
           ...
           for B26 in[2500,3000,3500,4000]:
            ...
            for m in range(12):
                ...
                if B14>130:
                    ...
                    ws=min(wDemand,max(0,B3-B6));wr=round(ws*B1*ws_p)
                ...
                wcogs=round(ws*B10*ws_i+ws*ws_pk)if ws>0 else 0
    ...
    p=best[1];m=best[2]
    print('ws=%.2f/%.2f/%.1f: %d/yr B1=%d B24=%d B14=%d B15=%d'%(ws_p,ws_i,ws_pk,best[0],p[0],p[1],p[4],p[5])+', B10=%d B26=%d'%(p[6],p[7]))d B10=%d B26=%d'%(ws_p,ws_i,ws_pk,best[0],p[0],p[1],p[4],p[5],p[6],p[7]))
    print('  months: %s'%m[:6])
